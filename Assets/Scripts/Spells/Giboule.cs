...
public class Giboule : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Time.time > makeitrain + 1)
        {
            ...
            GameObject Testplayer = GameObject.Find("MageTutorial");
            if (Testplayer != null)
            {
                Instantiate(GameObject.Find("MageTutorial").GetComponent<SortSimpleTuto>().Prefabs[1], position, Quaternion.identity);
            }
            else
            {
            ...
            if (Testplayer != null)
            {
                currentPrefabObject = GameObject.Instantiate(GameObject.Find("MageTutorial").GetComponent<SortSimpleTuto>().Prefabs[0]);
            }
            else
            {
                currentPrefabObject = GameObject.Instantiate(GameObject.Find("LOCAL Player").GetComponent<Sorts_simple>().Prefabs[0]);
            }
